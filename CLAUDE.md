...
TX power / SWR readout during FT8 TX (inspired by qFT8): query the QMX's onboard SWR/power measurement and show it in `ft8_status.c`'s status line (e.g. "TX 4.0W SWR 1.2") during a burst. Needs a new CAT query issued just before `TX;` or just after `RX;` — not during the burst, since `cat_poll_set_paused(true)` covers that whole window and the FA/MD/FW poll must not collide with it either.

Exclude-prefix filter for CQ-run auto-reply (inspired by qFT8's "exclude prefix" setting): settings-drawer text field for one or more callsign prefixes to skip when picking a CQ-run reply target, wired into the existing CQ-row filtering in `ft8_qso.c`/`ft8_screen_view.c`.
