...
SWR-protection auto-reset (inspired by qFT8, QMX firmware 1.03.000+): if SWR protection trips during/after an FT8 burst (detected via the TX power/SWR CAT query above), automatically cycle `TX;`/`RX;` to clear it instead of leaving the radio stuck.

TX clash/sequence warning (inspired by qFT8's red-sequence-number indicator): extend `ft8_find_clear_tone_hz()`'s occupied-bin bitmask to flag when our chosen TX tone collides with another active station's frequency, surfaced as a warning on the decode list/waterfall.
