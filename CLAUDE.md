...
**The boot log now LISTS every `/spiffs` file with its size** (`adif_log.c`), because "536 KB used" was not actionable: answering *which file* previously needed a rebuilt firmware, and this partition holds the QSO log, the LoTW certificate **and private key**, and the diag log. The structural fix — the diag log deserves its own partition — is **#192, and it is now DECIDED AGAINST**: the 2.81 MB free above `ota_1` is firmware headroom, not log storage. Operator, 2026-09-18: *"my first priority is kept - fw expendabilities and stack maintenance comes first"*. The app partition is at **47 KB free** (measured, `0xb9d0` of 4 MB) and one feature from a build failure, while the diag log has a working answer today in a microSD card. ⚠ Do not propose the free tail for anything else.
