...
**The boot log now LISTS every `/spiffs` file with its size** (`adif_log.c`), because "536 KB used" was not actionable: answering *which file* previously needed a rebuilt firmware, and this partition holds the QSO log, the LoTW certificate **and private key**, and the diag log. The structural fix — the diag log deserves its own partition, and 2.81 MB of flash is free above `ota_1` — is **#192, deliberately not done**: it changes the flash layout on shipped units.
