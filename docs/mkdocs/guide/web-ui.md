...
- **Clear log (delete all)** — deletes **all** logged QSOs from the Tab5. No undo, so it asks you to type `DELETE` to confirm — download the ADIF first if you want a copy. Handy before a POTA activation: start with an empty log and the ADIF at the end is exactly the file you submit
