...
is formatted for markdown. Operator has tested the fixes on hardware, so the
answers below state them plainly.
...
On when you can have all this: Everything below is done and tested, but none of it is in v1.8.2, so please do not go looking for it on the version you are running. It comes in the next release.
...
On the dark edges when zoomed: Fixed rather than just explained. The filter is now twice as long, which roughly halves the width of the darkened band at each edge. I did not widen the filter instead, because that would have traded the dark edge for false signals appearing where there is nothing.
...
On the distance column in the browser FT8 list: Built. The browser list now has KM and BRG columns after HZ, in the same order the Tab5 uses, and it follows the miles setting so the header reads MI if you have that ticked. The Tab5 works the distance out and sends it, rather than the browser calculating its own, so the two screens cannot disagree. Where a station has not sent a grid the column shows a dash rather than a made-up number. It comes in the next release.
