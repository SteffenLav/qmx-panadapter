...
**Tab5 Audio Decoding (Beta) + Pause Button.**

- **Panoramic CW Split** — Real-time frequency-based station separation in CW. Two stations pan left/right across the stereo stage with three live-tunable controls: Pan Width (frequency spread), Pan Blend (dry/wet mix), and Overlap (frequency range per station). All tunable during the QSO without reboot or mode change. Confirmed on air and ready for field testing.

- **Binaural CW** — First-generation audio separation. Works best with bandwidths 300 Hz and above; do not expect good separation below 200 Hz (fundamental audio bandwidth constraint, not an algorithm issue).

- **Resource Management window** — Tap and hold the top bar in the Panadapter view to open. Shows current resource usage and headroom. Audio features consume finite FPGA and memory resources; not all can run simultaneously. Users toggle audio, spectrum, waterfall, and decode on/off to balance the load.

- **Pause Button** for mid-QSO — Skip one transmission to check if the QSO slot is still open, then resume with the same message. Shown in the mid-QSO button row (alongside Re-send / RR73 / 73) during active FT8/FT4 exchanges (WAIT_RPT / WAIT_ROGER / WAIT_RR73).

- **Audio is BETA** — First-generation implementation subject to further development. Real-world feedback shapes the next iterations. See [Audio Features](docs/mkdocs/guide/audio.md) for detailed docs, bandwidth requirements, limitations, and future work (web I/Q streaming to your PC for local audio processing).

All audio and UI changes tested on hardware. v1.16.0 includes the taskLVGL PSRAM fix (internal RAM 7 KB → 50 KB) from v1.15.4 soaked overnight on the dev bench.

### v1.16.1 — 2026-09-22

**The resolving release: everything reported against v1.16.0 in its first day, plus one thing v1.16.0's own notes promised and never shipped.**

- **Pause button was never visible** (Randy N4OPI). It was built, wired and announced — and placed on a second row at y=610 when the left pane's content height is 600, so it was drawn outside the panel. Now the fourth button in the mid-QSO row, all four equal-sized and spanning the full column width. It also now appears **while calling CQ**, which is where Randy looked first: skipping one CQ call to listen is the manual form of the automatic listening slot, and refusing it there was an arbitrary limit.
- **Pause then did nothing when pressed.** `on_tx_complete()` re-arms the next message the instant a burst ends, so it is already ARMED for the whole listening window — exactly when somebody reaches for Pause. The flag was only read inside `rearm_current()`, below four early returns. The capture showed 7 presses, 1 consumption, 6 transmissions. Now the armed request is disarmed too, the flag is consumed above every other guard, and the button reads **Paused** in black on amber while armed.
- **A slot skipped by Pause no longer counts toward giving up on the contact.** The partner has nothing to answer, so the quiet slot looked like being ignored. The busy-partner hold already set the precedent ("not counting a miss").
- **The three panoramic CW controls had no user interface at all** — `pan_width`, `pan_blend` and `pan_overlap` existed only as a JSON action, while the release notes described them as adjustable while listening. Now real sliders in Resource Management, under Binaural CW.
- **A Gain control for the audio level** (Samuel W7STF, Bruce N9JCV). The low volume was the AGC's gain ceiling, not the volume control, and nothing could reach it. All four settings persist to NVS and are carried in a config backup; a config import applies them live.
- **SSB was unintelligible while CW was clear** (Gyula HA3HZ). The AGC runs on the 6 kHz decimated stream and its attack/release were never rescaled — 24 ms and 1.2 s instead of 3 ms and 150 ms, exactly the decimation factor. Every syllable onset clipped before the gain moved, then stayed ducked. ⛔ This fix existed before and was reverted for "no confirmed difference" — against CW clicking, the one signal that cannot exercise an AGC's rate.
- **WSPR transmit schedule anchored to UTC** (John W5JSS). A 20-minute schedule walked from the 0 and 2 minute marks to 2 and 4 after several hours. The period arithmetic was correct; the model was not — the next group was rolled from wherever the last one ended, so any missed cycle, clock step or restart re-anchored it permanently and silently. Now derived from the cycle index, so it cannot drift. Verified on the bench at 1 W: three transmissions, all exactly on the grid.
- **Resource Management layout** — Binaural CW grouped with RX Audio, the network feeds grouped together, and disabled sliders now visibly dimmed (`LV_STATE_DISABLED` blocks input but does not dim a slider, so they refused touches while looking live).
- ⚠ **Documented, not fixed: switching the QMX on while the Tab5 is still starting** starves its internal heap for the whole session — the web interface may become unreachable, and a crash is likelier. Wait until the spectrum is running and WiFi shows a network. Added to Quick Start and Troubleshooting.

Audio and binaural CW remain a **declared beta**.

### v1.16.2 — 2026-09-22

**The audio is finally listenable, and a good microSD card is no longer thrown away. Four measured root causes, and three hypotheses that died on the way.**

- **RX audio break-ups were a CLOCK MISMATCH, not CPU load.** Measured: the QMX delivers **47,885 pairs/s** over USB while the ES8388 plays exactly **48,000 samples/s** from the Tab5's own oscillator. Nothing reconciled them, so the DMA ring lost ~115 samples a second; a descriptor is 320 frames (~6.7 ms), so one came up empty roughly every 2.8 s and `auto_clear` filled it with **zeros, silently**. Every counter read clean — `I2Sovf` flat, `read to=0`, `clips=0`, `gap n=0` — which is why a year of CPU work never found it. Almost certainly also the "chirp every 4-5 s on a silent band" recorded in `rx_audio.c` long before. Fixed by holding the output at exactly `DSP_SAMPLE_RATE_HZ` against the wall clock, duplicating or dropping one sample pair per frame when the running total drifts a whole pair — one pair in ~420, ~21 µs of correction per second, self-tracking. ⭐ **Ask "are we feeding 48,000 samples a second?" before measuring anything about CPU.** Three hypotheses were flashed and falsified first: a 1 Hz debug log on `fft_task`, the I2S TX interrupt landing on core 0, and the write-duration counter (which measures the *design* — the blocking write IS the pacing mechanism).
- **A peak limiter after the AGC.** The AGC normalises to 30000 of 32767 — 92% of full scale — so every signal arrived near maximum and anything faster than its ~3 ms attack hit the hard clamp as clipping: loud *and* harsh. 17,692 clips were measured in one evening. ⛔ The fix is deliberately **not** a lower `agc_target`: Samuel W7STF is on the same firmware asking for more volume. Instantaneous attack, ~250 ms release, threshold 26000 (~2 dB under the clamp), one gain across both ears so the binaural image cannot shift.
- **LSB demodulated the wrong side of the dial** (Gyula HA3HZ). The QMX's LO sits 12 kHz below the dial, so the *sign* of a baseband offset says which side you are on. `filter_params_for_mode()` had only the positive case, so LSB played whatever was mirrored opposite the station. `ui.c`'s `compute_passband_edges_hz()` had the LSB edges right all along — the spectrum drew the correct passband while the audio listened elsewhere. ⚠ CW-R is the same shape and is **not** addressed: whether the QMX inverts the IQ itself there is not something the code can say.
- **A mounted microSD card was declared removed** (Dennis WN4FLA). The card mounted, `README.txt` wrote successfully, then the first append to `qmx-log.txt` returned EIO; five retries, `unmount("write failures")`, and because WiFi was up the no-card branch latched for the session. Measured across 547 boots: every `"wb"` file wrote fine on the same mount (README 265/265) while every append failed, and the card remounted **267 times** and failed again each time with 100 KB of DMA free. **The long-lived log handle was opened too early in `try_mount()`** — before `write_readme()` touched the volume. Opening it last fixes it. ⚠ What invalidates it inside FatFs is **not established**; the exFAT `fs->dirbuf` lifetime is a suspicion only, and it would explain why Gyula's "format it to FAT" advice worked.
- **The diagnostic log's 5 MB rotation was unreachable on every WiFi unit.** It lived only inside `mirror_diag()`, which stops seconds after boot at `park_snapshot()`; `mirror_diag_slow()`, the only writer afterwards, had no rotation at all. Worse, the rotate is what *frees* space and it was gated behind a successful write. The mount now reports free space on the first write failure.
- **`render` moved from core 0 to core 1.** `cpu_owners` with `ui_dev_canvas_hide()` showed `taskLVGL` 66.7% / `render` 13.5% with the canvases shown and 33.9% / 44.3% with them hidden: render *takes* 13.5% and *wants* 44.3%, starved by `taskLVGL` at priority 5 above it. Core 0 was being asked for **~134% of itself** while core 1 idled at 81%. Safe where the falsified `taskLVGL` move was not — priority 3 is below `fft_task`'s 4, and render touches neither LVGL nor `display_lock`. Result: `idle0` 0.0% → 11-14%, I2S write max 185 ms → 33 ms. It did **not** fix the break-ups, because they were never CPU.
- **Pause reached the web UI** (Randy N4OPI, who works his station remotely and rarely touches the Tab5). Dispatched to the same `ft8_qso_pause_next_tx()` the Tab5 button calls. It is now a **toggle**, it reports its state through `/api/status` so the browser button can paint itself, and both screens use play/pause transport glyphs — icon only and 32 px on the Tab5.
- **A pause no longer eats the QSO timeout** (Gyula HA3HZ). The v1.16.1 grace forgave exactly one slot, which only balances a single press; `register_miss()` now forgives per pause and also while a pause is still pending.
- **Reboot the Tab5 from the web UI** (Randy N4OPI). The body already existed as `ota_reboot` and was simply unreachable without a pending update.
- **KM and AGE overlapped by 10 px** on the FT8 decode list (Gyula HA3HZ) — measured from the constants, both columns moved into the unused right margin, neither narrowed.
- **A 1 Hz `ESP_LOGI` removed from `fft_task`**, the audio ring's sole consumer. Its own comment claimed it was "never in normal use"; the operator sits at ×2 zoom, where it fired every second.

⚠ **Known:** audio still stutters while a browser has the panadapter open — core 0 has only 11-14% idle with the radio streaming and the WebSocket push tips it over. Measured, and next. Audio and binaural CW remain a **declared beta**.
