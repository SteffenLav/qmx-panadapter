...
- **⛔ Emergency fix: v1.14.0 crashed on tuning or on QMX power-on** *(Martin Howard; Rick Trommer W5NR, on the groups.io list within a day of v1.14.0 shipping)*. `spots_any_source_enabled()` copied the whole settings struct onto a stack as small as 4096 bytes (the render task, and independently the CAT poll task — either one could be hit by nearly every frequency change), overflowing it on almost any tuning motion or the frequency update an immediate QMX reconnect sends. Fixed by Uwe DL8UG: four narrow getters reading one bool each, instead of the whole struct. Hardware-verified: tuning across the band with the spots lane actively repainting, zero crashes.
