...
| 25 | Freq-entry popup: selectable size + persistent (non-centered) parking location (Samuel W7STF, groups.io #172940, Jun 30) | ❌ Not started | Medium | The direct frequency-entry keypad (`ui.c`, `freq_buf_to_hz`/`freq_apply_key` path) always opens centered on screen at a fixed size. Samuel wants (a) a selectable/smaller size and (b) for it to remember the last screen position instead of re-centering every time. (b) is the simpler half — persist last X/Y to NVS (same sticky-setting pattern as per-band freq recall, see `feedback_per_band_freq_recall`) and restore on open; (a) needs the keypad layout to support more than one fixed size, larger scope |
