...
>
> **New in v1.8.2 — your radio's own spurs can be removed from the display, and a POTA clock that stopped being stolen.** If you have ever seen evenly spaced signals that sit in the same place, never move when you tune, and are still there with the antenna unplugged, those come from the QMX's own synthesizer — on the bench at 14.074 MHz the strongest was nearly 40 dB above the noise floor. **Spur suppression** finds them by nudging the dial 25 Hz for two seconds, because a real signal stays put while these move sixteen to fifty times further; it can then subtract them or erase them, and it marks in teal whatever it touches. **Off by default**, under Settings → Waterfall. A **QMX without GPS no longer overwrites an accurate clock** — switching the radio on at a POTA site used to replace good UTC with its power-on 00:00, and now the Tab5's own RTC wins and sets the radio instead. **RIT can be parked** with a long press and restored unchanged, for a net where one station is off frequency, and its offset is printed beside its marker on the waterfall. The **band strip stays visible out of band** instead of leaving an empty row. And the panadapter no longer **switches your radio off** trying to recover a USB fault only a QMX power cycle can clear. Full detail in [docs/version-history.md](docs/version-history.md).
>
...
Prefer a single printable file? [Download the User Guide PDF](docs/QMX-Panadapter-UserGuide-v1.8.2.pdf).
