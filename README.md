...
> **Release — v1.3.5.** A complete, self-contained FT8/FT4 station — receive, transmit, auto-QSO, ADIF logging, and upload to **all three major logbooks: QRZ, eQSL, and ARRL LoTW** — with no PC in the loop.
>
> **New in v1.3.5** (the week's field reports, from Don WB0LQW, Dennis WN4FLA, Roy KI0ER and Randy N4OPI):
>
> - **CQ now stops calling after a limit you set** — long-press Call CQ and the preset editor's new **CQ stop** button cycles never / 1 / 2 / 3 / 4 / 5 / 10 calls, applied the moment you tap it. While calling, the TX status counts **"call 2 of 4"** live, and after the last unanswered call the Tab5 listens through one more slot (an answer to your final call still starts the QSO normally) before going idle — Don's "I usually send CQ 2-4 times and then pause," made automatic.
> - **The QSO log opens in your browser**: QSO Logs → **View / edit log** shows the whole log as a table — **click any column header to sort** (date-sorting groups an activation's QSOs), a delete cross on every row, and a **Delete all** that asks you to type `DELETE`.
> - **Delete all exists on the Tab5 too**, in the ADIF log viewer — two taps, with the first arming it and showing the live count ("ALL 34?").
> - **The busy-station hold has an exit now** (Roy KI0ER): v1.3.3's polite wait while your pounce target works someone else had no cancel — the one state where you most want to opt out was the one state without a way to. The status line now shows **TAP TO CANCEL**, and tapping frees you to work anyone else, with the abandoned exchange resumable for a few minutes.
> - **The settings drawer stops stealing your finger** — a grabbed slider or checkbox stays grabbed instead of the drawer scrolling out from under it, and the slider knobs' catch area more than doubled.
> - The **QMX volume slider tops out at 50 dB** on Randy's third and best-informed report (the earlier "40 is not loud enough" turned out to be an antenna-off reading — what was missing was band noise, not gain), and his side-by-side check confirms **the Tab5's dB figure reads identical to the QMX's own LCD**.
> - A latent bug fixed before it bit anyone: clearing the log never reset the QRZ/eQSL/LoTW upload positions, so every QSO logged after a clear would have been silently skipped by all three uploads.
>
...
**v1.3.4 is here** — the same-day field reports on v1.3.3, all of them real: a QSO is **no longer finished just because we said 73** (a partner who never decoded your final now gets it re-sent, instead of the Tab5 moving on while they are still calling you), finishing by hand no longer **duplicates the log entry**, an invented **`599`** report is no longer written into your log when none was exchanged, the **occupancy map is filtered by transmit window**, the **QMX volume slider is confirmed working on a real radio** and now spans a usable 0–40 dB, and your **TX frequency is a permanent button on the main screen** with a live mini occupancy strip and **TX Hold**. WiFi strength in the bottom bar is a fan icon rather than a dBm figure. Roy KI0ER and Randy N4OPI between them found all of it. On top of v1.3.3's tone picker and busy-station hold, v1.3.2's grid-square fix and built-in User Manual, v1.3.1's DT/HZ decode columns, and v1.3.0's intelligent Transmit. Next on the bench:
