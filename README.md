...
> **Release — v1.7.1.** A complete, self-contained FT8/FT4 station: spectrum and waterfall, on-device decode and transmit, automatic QSOs, ADIF logging, and upload to **all three major logbooks — QRZ, eQSL and ARRL LoTW** — with no PC in the loop. It runs offline for POTA/SOTA, streams to any browser on the LAN, and carries its own user manual inside the firmware.
>
> **v1.7.1 is a bug-fix patch — tried a Bluetooth mouse on v1.7.0? Install this so your WiFi doesn't go flaky.** Switching the mouse on could leave WiFi dropping and recovering, with the web UI flapping between Connected and Disconnected. The two look unrelated, which is exactly why it was hard to find — WiFi and Bluetooth share one link to the wireless co-processor, and the Tab5 was listening for every Bluetooth device in the building, continuously. It now listens in short bursts and ignores everything that is not your mouse. Also: the settings drawer closes when you tap outside it, the CW transmit offset has ±10/±50 buttons, and DX cluster spots stop vanishing every minute (all from Michael KZ4LY). Full list in [docs/version-history.md](docs/version-history.md).
>
...
Prefer a single printable file? [Download the User Guide PDF](docs/QMX-Panadapter-UserGuide-v1.7.0.pdf).
