...
> **Release — v1.16.4.** A complete, self-contained FT8/FT4 station: spectrum and waterfall, on-device decode and transmit, automatic QSOs, ADIF logging, and upload to **four logbooks — QRZ, eQSL, ARRL LoTW and your own Cloudlog or Wavelog** — with no PC in the loop. It runs offline for POTA/SOTA, streams to any browser on the LAN, and carries its own user manual inside the firmware.
>
> **v1.16.4 fixes four faults operators found in their own shacks.** **A station answering your CQ is answered in the very next slot — every time.** Decoding is split across both processors and only one half could act on a caller, so the same situation replied immediately about half the time and a cycle late the rest *(Gyula HA3HZ)*. **Waiting for a busy station now really is capped at six minutes** — the counter reset instead of latching, so it could wait over half an hour, and never got back to calling them either. **Headphones silence the internal speaker** *(Roy KI0ER)*. **Resource Management is renamed Audio Settings** and opens from its own button in the settings drawer. **Preferred network actually decides which WiFi you join** *(Randy N4OPI)* — it was only consulted after two failed connects. **A restart caused by a power brownout says so** *(Samuel W7STF)*. And the long-running **SD "dot goes yellow and writes stop" is root-caused**: the card driver is refused a memory buffer, so the write never reaches a card that is mounted and healthy.
>
...
Prefer a single printable file? [Download the User Guide PDF](docs/QMX-Panadapter-UserGuide-v1.16.4.pdf).
